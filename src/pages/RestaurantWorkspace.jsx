...
import { getOrderStatuses, ORDER_STATUSES, useApp } from '../context/AppContext';
...
export default function RestaurantWorkspace() {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      ...
      if (!order) return;
      const statusPath = getOrderStatuses(order);
      const current = statusPath.indexOf(order.status);
      if (current >= statusPath.length - 1) { setAutopilot(false); return; }
      updateOrderStatus(order.id, statusPath[current + 1]);
    }, 4200);
  ...
  return <main className="workspace" id="main">
    ...
    <section className="workspace-main">
      ...
      {tab === 'orders' && <><div className="workspace-kpis"><Metric icon={Activity} label="Active orders" value={activeOrders.length} /><Metric icon={Clock3} label="Average fulfilment" value="18m" /><Metric icon={Gauge} label="Tonight revenue" value={`€${revenue.toFixed(0)}`} /></div><div className="autopilot"><div><Play /><span><b>Presentation autopilot</b><small>Advance one selected order through the real status pipeline.</small></span></div><select value={autoOrder} onChange={(e) => setAutoOrder(e.target.value)}><option value="">Select order</option>{activeOrders.map((order) => <option value={order.id} key={order.id}>{order.id}</option>)}</select><button className="button button-quiet" disabled={!autoOrder} onClick={() => setAutopilot((value) => !value)}>{autopilot ? <><Pause />Pause</> : <><Play />Start</>}</button></div><div className="order-board">{columns.map((status) => <section className="order-column" key={status} onDragOver={(event) => event.preventDefault()} onDrop={(event) => updateOrderStatus(event.dataTransfer.getData('text/order-id'), status)}><header><span>{localize(orderStatusCopy[status], lang)}</span><b>{orders.filter((order) => order.status === status).length}</b></header>{orders.filter((order) => order.status === status).map((order) => { const statusPath = getOrderStatuses(order); const statusIndex = statusPath.indexOf(status); return <article draggable onDragStart={(event) => event.dataTransfer.setData('text/order-id', order.id)} key={order.id}><div><GripVertical /><strong>{order.id}</strong><time>{new Intl.DateTimeFormat(lang, { hour: '2-digit', minute: '2-digit' }).format(new Date(order.createdAt))}</time></div><p>{order.items.length} items · {order.type}</p><span>€{Number(order.total).toFixed(2)}</span><div className="order-card-actions">{statusIndex > 0 && <button onClick={() => updateOrderStatus(order.id, statusPath[statusIndex - 1])}>Back</button>}{statusIndex < statusPath.length - 1 && <button onClick={() => updateOrderStatus(order.id, statusPath[statusIndex + 1])}>Advance</button>}</div></article>; })}</section>)}</div></>}
