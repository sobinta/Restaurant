...
export default function WorkspaceShell({ role }) {
  ...
  return <main className="workspace-shell-page"><header className="workspace-shell-bar"><Link to="/">ARSHIDA</Link><WorkspaceSwitcher branchId={branchId} role={effectiveRole} /><Link to="/account">{copy.account}</Link></header><section className="workspace-shell-stage page-width"><span className="workspace-code">{meta.code}</span><div className="workspace-shell-icon"><Icon /></div><span className="eyebrow">SECURE WORKSPACE · {branchId === 'global' ? 'GLOBAL' : copy.branch}</span><h1>{copy[effectiveRole]}</h1><p>{copy.coming}</p>{(effectiveRole === 'manager' || effectiveRole === 'superadmin') && branchId !== 'global' && <Link className="button button-primary workspace-team-link" to={`/workspace/${branchId}/owner/staff`}>Team access</Link>}<div className="workspace-coming"><Construction /><span>Foundation active</span><b>Realtime-ready · RLS protected</b></div></section></main>;
}
